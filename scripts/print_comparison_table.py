...
for snp_threshold in range(1,1001):
	...
	if (ATCC_PRECISION + ATCC_RECALL) == 0:
		ATCC_FSCORE = 0
	else:
		ATCC_FSCORE = 2 * ((ATCC_PRECISION * ATCC_RECALL) / (ATCC_PRECISION + ATCC_RECALL))
	
	K12_PRECISION = K12_TP / (K12_TP + K12_FP)
	K12_RECALL = K12_TP / 19
	if (K12_PRECISION + K12_RECALL) == 0:
		K12_FSCORE = 0
	else:
		K12_FSCORE = 2 * ((K12_PRECISION * K12_RECALL) / (K12_PRECISION + K12_RECALL))
	
	DSM_PRECISION = DSM_TP / (DSM_TP + DSM_FP)
	DSM_RECALL = DSM_TP / 19
	if (DSM_PRECISION + DSM_RECALL) == 0:
		DSM_FSCORE = 0
	else:
		DSM_FSCORE = 2 * ((DSM_PRECISION * DSM_RECALL) / (DSM_PRECISION + DSM_RECALL))
